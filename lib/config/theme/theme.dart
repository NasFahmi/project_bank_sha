import 'package:flutter/material.dart';

//! splashScreen & onboard

const Color blackColorOnboard = Color(0xff020518);
const Color grayBackgroundColorOnboard = Color(0xffF6F8FB);
const Color whiteColor = Color(0xffffffff);
const Color tittleColorFont = Color(0xff14193F);
const Color secoundaryColorFont = Color(0xffA4A8AE);
const Color blueColorCarousel = Color(0xff53C1F9);
const Color grayColorCarousel = Color(0xffF1F1F9);
const Color btnPrimaryColor = Color(0xff5142E6);
const Color btnSecoundryColor = Color(0xff696B76);
